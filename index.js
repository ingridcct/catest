...
router.get('/get/html', function(req, res) {
    ...
    let xml = fs.readFileSync('championsleague.xml', 'utf8'), //read in the XML file
        xsl = fs.readFileSync('championsleague.xsl', 'utf8'); //read in the XSL file
...
router.post('/post/json', function(req, res) {
    ...
    function appendJSON(obj) {
        ...
        XMLtoJSON('championsleague.xml', function (err, result){
            if (err) throw (err);

            result.menu.section[obj.sec_n].entry.push({'match': obj.match, 'price': obj.price});

            console.log(JSON.stringify(result, null, " "));

            JSONtoXML('championsleague.xml', result, function(err){
