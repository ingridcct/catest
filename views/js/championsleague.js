...
function calculateBill(idMenuTable) {
    ...
    for (i = 0; i < aCBTags.length; i++) {
        // is this menu match selected? it is if the checkbox is checked
...
// This function either turns on or off the row highlighting for kit
// matches (depending on the value of bShowVeg)
function highlightKit(idTable, bShowVeg) {
    // if bShowVeg is true, then we're highlighting kit
    ...
    for (i = 0; i < aTRs.length; i++) {
        if (aTRs[i].getAttribute('kit') && aTRs[i].getAttribute('kit') == "true") {
            if (bShowVeg) {
...
window.addEventListener("load", function () {
    ...
    document.querySelector("#showVeg").addEventListener("click", function () {
        highlightKit('menuTable', this.checked);
    });
});